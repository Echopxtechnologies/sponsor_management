...
/**
 * Get paginated university students for DataTables
 */
/**
 * Get paginated university students for DataTables
 */
public function get_university_students_paginated($start, $length, $search = '', $order_column = 'name', $order_dir = 'ASC')
{
    $this->db->select('us.*, 
                       un.name as university_name, 
                       up.name as program_name,
                       sr.name as all_sponsor_names,
                       sr.id as sponsor_record_id', false);
    $this->db->from(db_prefix() . 'university_students us');
    $this->db->join(db_prefix() . 'university_name un', 'un.id = us.university_name_id', 'left');
    $this->db->join(db_prefix() . 'university_program up', 'up.id = us.university_program_id', 'left');
    $this->db->join($this->tbl_sponsor . ' sr', 'sr.id = us.sponsor_id', 'left');
    ...
    // Handle order column - prefix with us. for student fields
    $student_fields = ['id', 'name', 'email', 'contact_no', 'university_year_of_study', 'staff_id', 'active'];
    if (in_array($order_column, $student_fields)) {
        $order_column = 'us.' . $order_column;
    }
    ...
    // Add sponsor_count in PHP instead of SQL
...
/**
 * Count all university students
 */
public function count_all_university_students()
{
    return $this->db->count_all(db_prefix() . 'university_students');
}

/**
 * Count filtered university students
 */
public function count_filtered_university_students($search)
{
    $this->db->from(db_prefix() . 'university_students us');
    $this->db->join(db_prefix() . 'university_name un', 'un.id = us.university_name_id', 'left');
    $this->db->join(db_prefix() . 'university_program up', 'up.id = us.university_program_id', 'left');
